...
def parse_customer(r)
  ...
  (c.phone = r.customer_phone1) || r.customer_phone3
  ...
  c.remarks.create remark: r.customer_notes if r.customer_notes.present?
  c
end
...
def parse_subscriber(r, true_customer = false)
  ...
  if r.subscriber_contact_person.present?
    if r.subscriber_contact_person =~ /zastonj/i
      s.remarks.create remark: 'Zastonj'
    elsif r.subscriber_contact_person =~ /.+\@.+\..+/i
      s.email = r.subscriber_contact_person.downcase.strip
      s.save
    elsif r.subscriber_contact_person =~ /\d{2,3}[\/\-]{1}.*/i
  ...
  s.remarks.create remark: r.subscriber_notes if r.subscriber_notes.present?
...
Legacy.all.reject { |r| r.customer_id == 16 }.group_by(&:customer_id).each do |_cid, r|
  # CUSTOMER
